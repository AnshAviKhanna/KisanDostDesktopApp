...
// Updated to accept a directory path from Flutter
type ProcessRequest struct {
	ImagePaths  []string `json:"image_paths"`
	DatabaseDir string   `json:"database_dir"`
}

// Response body to Flutter (no changes)
type ProcessResponse struct {
	DatabasePath string `json:"database_path"`
	SessionID    string `json:"session_id"`
}
...
func processingHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Use the directory path provided by the Flutter client
	if req.DatabaseDir == "" {
		http.Error(w, "DatabaseDir path cannot be empty", http.StatusBadRequest)
		return
	}

	timestamp := time.Now().Format("2006-01-02_15-04-05")
	sessionID := fmt.Sprintf("session_%s", timestamp)
	newDbPath := filepath.Join(req.DatabaseDir, sessionID+".db")

	log.Printf("[Go] Created new session DB at: %s", newDbPath)

	// Get path to the python executable, assuming it's in the same folder
	exePath, err := os.Executable()
	if err != nil {
		http.Error(w, "Could not find executable path", 500)
		return
	...
	exeDir := filepath.Dir(exePath)
	pythonEnginePath := filepath.Join(exeDir, "dist/process")
